...
<?php printf("   <input type=\"hidden\" name=\"codigo\" value=\"%s\">\n", $codigo) ?>
  ...
  <select name="divisa" onChange="submit()">
<?php  echo lista_divisas($conn, $divisa) ?>
    </select>
</form>
</th>
...
<?php
  ...
  for ($i=0; $i<$num_ren; $i++) {
    ...
    $modif_divisa = $tipo_cambio[$ren->divisa] / $tipo_cambio[$divisa];
    $costo_divisa = $ren->costo1 * $modif_divisa;
    ...
    echo "   <input type=\"hidden\" name=\"boton\" value=\"costos\">\n";
    echo $hidden_vars;
    echo $hidden_rvars;
