...
<head>
  ...
  <title>OsoPOS - Factur Web v. <?php echo $factur_web_vers ?></title>
...
<?php
  ...
  if ($action == "muestra"  ||  $action == "agrega") {
    if ($action == "muestra") {
      ...
      $form_action = "$_SERVER['PHP_SELF']?table=$table&order_by=$order_by&action=cambia&offset=$offset&order=$order";
    }
    else if ($action == "agrega") {
      $val_submit = "value=\"Agregar producto\"";
      $form_action = "$_SERVER['PHP_SELF']?table=$table&order_by=$order_by&action=inserta&order=$order";
    }
...
?>
    ...
    <td><input type="text" name="fecha" maxlength="10" size="10" <?php echo $val_fecha ?> /></td>
    <td>R.F.C.</td>
    <td>
    <input type="text" name="rfc" size="13" maxlength="13" <?php echo $val_rfc ?> /></td>
   </tr>
   <tr>
     <td>Subtotal:</td>
     <td><input type="text" name="subtotal" size="10" <?php echo $val_subt ?> /></td>
     <td>I.V.A.</td>
     <td><input type="text" name="iva" size="10" <?php echo $val_iva ?> /></td>
<?php
    echo "  <td>Total</td>\n";
  ...
  if ($action == "cambia") {
    ...
    if (!$result = db_query($query, $conn)) {
      ...
      echo "<div class=\"mens_inf\">Factura $id $rfc actualizada.</div>\n";
    }
  }
  if ($action == "borrar") {
    ...
    if (!$result = db_query($query, $conn)) {
      ...
      echo "<div class=\"mens_inf\">Art&iacute;culo $codigo eliminado.</div>\n";
    }
  }
...
/*  if (isset($id_dept) || isset($id_prov))
  ...
  echo "  <td>\n";
  if ($offset > 0) {
    echo "<a href=\"$_SERVER['PHP_SELF']?offset=" . sprintf("%d", $offset-$limit);
...
<?php }
    ...
    echo "  <th><a href=\"$_SERVER['PHP_SELF']?offset=0&order_by=id&order=";
    printf("%d&table=%s",  $order_by=="id" && !$order, $table);
    echo "$href_prov\">Folio</a></th>\n";

    echo "  <th><a href=\"$_SERVER['PHP_SELF']?offset=0&order_by=fecha&order=";
    printf("%d&table=",  $order_by=="fecha" && !$order, $table);
    echo "$href_prov\">Fecha</a></th>\n";

    echo "<th>";
    echo "<a href=\"$_SERVER['PHP_SELF']?offset=0&order_by=rfc&order=";
    printf("%d&table=%s",  $order_by=="rfc" && !$order, $table);
    echo "$href_prov\">R.F.C.</th>\n";

  if ($table == "facturas_ingresos") {
    echo "<th>";
    echo "<a href=\"$_SERVER['PHP_SELF']?offset=0&order_by=razon_soc&order=";
    printf("%d&table=%s",  $order_by=="razon_soc" && !$order, $table);
    echo "$href_prov\">Nombre o razón social</th>\n";
  }

    echo "  <th><font face=\"helvetica,arial\"><a href=\"$_SERVER['PHP_SELF']?offset=0&order_by=subtotal&order=";
    printf("%d&table=%s",  $order_by=="subtotal" && !$order, $table);
    echo "$href_prov\">Subtotal</a></font></th>\n";

    echo "  <th><font face=\"helvetica,arial\"><a href=\"$_SERVER['PHP_SELF']?offset=0&order_by=iva&order=";
    printf("%d&table=%s",  $order_by=="id_prov" && !$order, $table);
    echo "$href_prov\">I.V.A.</a></font></th>\n";

    echo "  <th><font face=\"helvetica,arial\"><a href=\"$_SERVER['PHP_SELF']?offset=0&order_by=total&order=";
    ...
    for ($i=0; $i<$num_ren; $i++) {
      ...
      if (!($i%4) || $i==0)
        $td_fondo = " class=\"estilo_a\"";
      else if (!(($i+2)%2))
        $td_fondo = " class=\"estilo_b\"";
      else
        $td_fondo = " class=\"estilo_c\"";
      ...
      printf("<a href=\"$_SERVER['PHP_SELF']?id=%s&table=%s", $reng->id, $table);
      echo "&order_by=$order_by&order=$order&action=muestra&offset=$offset$href_prov\">";
      printf("%s</a></font></td>\n", $reng->id);
      echo "  <td><font face=\"helvetica,arial\">$fecha</font></td>\n";
      echo "  <td><font face=\"helvetica,arial\">$reng->rfc</font></td>\n";

      if ($table == "facturas_ingresos")
        echo "  <td><font face=\"helvetica,arial\">$razon_soc</font></td>\n";
...
<?php
    ...
    echo "<div class=\"mens_inf\">No hay facturas que coincidan en la base de datos</div>\n";
  }
